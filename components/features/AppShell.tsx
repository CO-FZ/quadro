...
export default function AppShell({ profile, children }: AppShellProps) {
  ...
  const isManager = isAdmin || profile?.role === 'coordenador'

  const navItems = [
    ...NAV_ITEMS,
    ...(isManager ? [{
      href: '/admin',
      // Admin gerencia usuários/whitelist/auditoria/férias; coordenador só vê Férias.
      label: isAdmin ? 'Usuários' : 'Férias',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
                ...
                {menuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-card border border-border rounded-xl shadow-xl py-2 z-50">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-2 px-4 py-2 text-sm text-foreground hover:bg-muted transition-colors w-full"
                    >
